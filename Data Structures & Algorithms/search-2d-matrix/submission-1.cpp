...
public:
    bool searchMatrix(vector<vector<int>>& matrix, int target) {
        ...
        while (left < right) // Converge binary search making left==right after loop
        {
            int mid = left + ((right-left+1)/2); // left = mid;
            // int mid = left + ((right-left)/2); // right = mid;
        ...
        while (left <= right) // answer binary search
        {
